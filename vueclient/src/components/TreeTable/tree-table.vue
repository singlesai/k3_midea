<template>
  <div class="tree-table" style="overflow: auto;">
    <div class="tree-head">
      <table style="width: max-content;">
        <thead>
        <tr>
          <td v-for="(head, k) in header" :key="k" class="td" :style="{'width':head.width+'px'}">
            {{head.caption}}
          </td>
        ...
        <!--tbody>
          <tr>
            ...
            <tree-item
                v-for="(model,i) in treeDataSource" :key="'root_node_'+i"
                :root="0"
                :num="i"
                :header="header"
...
export default {
  ...
  methods: {
      ...
      let reduceDataFunc = (data, level) => {
        data.map((m, i) => {
          if (this.header === undefined) {
            this.header = {}
            for (var k in m) {
              if (this.header[k] === undefined) {
                this.header[k] = {caption: k}
              }
            }
    ...
    getMore () {
      alert('滚动到底部加载更多')
      // 滚动到最后
      document.getElementsById('scrollWrap').mCustomScrollbar('scrollTo', 'top', {
        scrollInertia: 0
      })
    },
    // 编辑
